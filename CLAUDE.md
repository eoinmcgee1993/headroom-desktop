...
- Do NOT port upstream #3380's floor semantics into the pinned wheel's overlay again. The floor is correct inside upstream's full code path; against our pin it is a PARTIAL replay that leaves every byte past the confirmed floor free to bust. v0.35.0 has no size bound at all (`git show v0.35.0:headroom/cache/prefix_tracker.py | grep -c _compact_json_bytes` -> 0), and full replay is what reproduces its behaviour.
