...
- Diff `headroom/transforms/` between the pins. This is the compression ENGINE and nothing above covers it. The 0.35.0 -> 0.37.0 bump changed it by +421 lines (`content_router.py` +174, `mixed_content.py` +101, `kompress_compressor.py` +59, `compression_policy.py` +55) and shipped unexamined, because the rules only named the savings/metrics/server files. Measured consequence, same model and same size band so workload is controlled: fable-5 500k+ compression fell from 4.64% (25,631 tok/request) to 0.45% (2,463) across the swap. Prime suspect is the net-cost model going TTL-aware -- `CACHE_WRITE_MULTIPLIER_1H = 2.0` is new in 0.37.0, and this app runs 1h TTL (`cache_write_1h_tokens` nonzero, `cache_write_5m_tokens` zero), so every compression decision is now priced against a 2.0x cache-write premium instead of 1.25x and far more of them decline. Whether that is economically correct is UNRESOLVED: it needs `prefix_cache.by_provider.*.uncached_input_tokens` from /stats, because the cost decomposition cannot be derived from PERF logs (`tok_after` and `cache_read` come from different tokenizers and must never be differenced).
