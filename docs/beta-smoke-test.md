...
From 0.9.3 on, the app takes this snapshot itself: on the first launch of a new version, `snapshot_state_on_version_change` (storage.rs) copies the three state files raw into `config/pre-update/` - before anything parses them - with a `meta.json` naming the from/to versions. When the build being replaced is >= 0.9.3, verify `meta.json`'s `from_version` is that build and diff against `config/pre-update/` instead of a manual snapshot. The manual block below remains for upgrades from older builds and as a cross-check.
